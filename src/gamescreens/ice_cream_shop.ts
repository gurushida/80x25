...
import { createFullHotspot, Hotspots, HotspotMap, combine } from "../hotspots";
import { dog } from "../animation/animations";
import { Action } from "../actions";
import { GameScreen } from "./gameScreen";
import { ice_cream_shop } from "../background";

const fullFilter = createFullHotspot(Hotspots.ICE_CREAM_SHOP);
const doorFilter = (x: number, y: number) => {
    if (x >= 32 && x <= 41 && y >= 8 && y <= 14) {
        return Hotspots.ICE_CREAM_SHOP_DOOR;
    }
...
iceCreamShopHotspotMap.set(Hotspots.ICE_CREAM_SHOP_DOOR, { description: 'Enter ice cream shop', isMovementHotspot: true })
